...
class CoreText: NSObject {
    ...
    class func GetRangeRects(range:NSRange, frameRef:CTFrame?, content:String? = nil) -> [CGRect] {
        ...
        for i in 0..<lineCount {
            ...
            if lineRange.rhs > range.location , lineRange.location < range.rhs{
                
                contentRange.location = max(lineRange.location, range.location)
                
                let end = min(lineRange.rhs, range.rhs)
            ...
            if contentRange.length > 0 {
                ...
                let xEnd:CGFloat = CTLineGetOffsetForStringIndex(line, contentRange.rhs, nil)
